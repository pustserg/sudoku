...
func TestCandidateMask(t *testing.T) {
	...
	// 5,3,7; column 2 already has 4,8; box 0 already has 5,3,6,9,8. The
	// only digit consistent with all three is 1.
	mask := candidateMask(g, 0, 2)
	want := uint16(1) << 1
	if mask != want {
		t.Errorf("candidateMask(0,2) = %09b, want %09b", mask, want)
	}
	if got := candidateDigits(mask); len(got) != 1 || got[0] != 1 {
		t.Errorf("candidateDigits(%09b) = %v, want [1]", mask, got)
	}
	if got := popcount(mask); got != 1 {
		t.Errorf("popcount(%09b) = %d, want 1", mask, got)
	}
}
```
